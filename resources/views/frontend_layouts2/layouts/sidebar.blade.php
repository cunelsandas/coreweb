...
<div class="container-fluid" style="padding: 0px">
            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-darker hidden" style="width: 780px;margin-top: 5%">
                ...
                <div class="collapse navbar-collapse" id="main_nav" style="position: fixed">
                    ...
                    <ul class="navbar-nav">
                        ...
                        @foreach(getAllMenu('menus_top') as $key => $value)
                            @if(count($value->sub) == 0)
                                <li class="nav-item {{count($value->sub) != 0 ? 'has-megamenu':''}}" style="border-radius: 25px;font-size: 1rem;" onMouseOver="this.style.backgroundColor='{{$css->web_nav_bgcol}}'"  onMouseOut="this.style.backgroundColor=''">
                                    <a class="nav-link" style="font-weight: 700; target="{{$value->target == 1 ? '_blank ': '_self'}}" href="{{count($value->sub) != 0 ? 'javascript:;': url($value->url)}}">{{$value->name}} </a>
                                </li>
                            @elseif(count($value->sub) != 0)
                                <li class="nav-item {{count($value->sub) != 0 ? 'has-megamenu':''}}" style="border-radius: 25px;font-size: 1rem;" onMouseOver="this.style.backgroundColor='{{$css->web_nav_bgcol}}'"  onMouseOut="this.style.backgroundColor=''">
                                    <a class="nav-link" href="#" style="font-weight: 700" data-toggle="dropdown" target="{{$value->target == 1 ? '_blank ': '_self'}}" href="{{count($value->sub) != 0 ? 'javascript:;': url($value->url)}}">{{$value->name}} </a>
                                    <div class="dropdown-menu megamenu" role="menu">
                                        <div class="row">
                                            @foreach($value->sub as $index => $item)
                                                <div class="col-md-3">
                                                    <div class="col-megamenu">
                                                        <a class="nav-link" style="font-size: 1rem;font-weight: bolder" target="{{$item->target == 1 ? '_blank ': '_self'}}"
                                                           href="{{count($item->sub) != 0 ? 'javascript:;': url($item->url)}}">{{$item->name}}</a>
                                                        @if(count($item->sub) != 0)
                                                            <ul class="list-unstyled">
                                                                @foreach($item->sub as $k => $v)
                                                                    <li style="margin-left: 5px;font-size: 14px"><a href="{{url($v->url)}}"> - {{$v->name}}</a></li>
                                                                @endforeach
                                                            </ul>
                                                        @endif
    ...
    <nav id="web-nav" class="navbar navbar-expand-lg navbar-dark fixed-top" style="width: 100%;z-index: 10;margin-top: 1%">
