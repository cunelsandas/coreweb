<script>
        ...
        methods: {
            getData: function (page) {
                ...
                axios.post(url, data).then((res) => {
                    ...
                    this.$nextTick((e) => {
                        sortA();
                        $('.card-img-overlay').css('cursor', 'grab');
                    })
                }).catch(() => {
    ...
    let sortA = () => {
        $('.connectedSortable').sortable({
            ...
            connectWith: '.card-move',
            // handle: ".connectedSortable",
            forcePlaceholderSize: true,
            zIndex: 999999,
            update: function (event, ui) {
                let id = $(this).sortable("toArray", {attribute: 'data-id'});
                let block = $(this).sortable("toArray", {attribute: 'data-block'});
                console.log(id)
                console.log(block)
                /*var itemOrder = $('.connectedSortable').sortable("toArray");
                var itemOrderNew = $('.connectedSortable').sortable("toArray", {attribute: 'data-list'});
                for (var i = 0; i < itemOrder.length; i++) {
                    console.log("Position: " + i + " ID: " + itemOrder[i] + "LIST :" + itemOrderNew[i]);
                }*/
            },
            scroll: false,
        });
